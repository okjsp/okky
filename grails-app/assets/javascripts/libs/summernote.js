...
(function (factory) {
    ...
    var Editor = function () {
         ...
         * @param {String} sUrl
         */
        this.insertCodeBlock = function ($editable, langType, isShowLineNumbers) {
            recordUndo($editable)
            range.create().insertNode($('<br/>').get(0));
            range.create().insertNode($('<pre class="'+(isShowLineNumbers ? 'line-numbers':'')+'"><code class="language-'+langType+'">\n\n\n</code></pre>').get(0));
            range.create().insertNode($('<br/>').get(0));
        };
         ...
         * @return {Object}
         */
        this.getLanguageType = function ($editable) {
            $editable.focus();
    ...
    var Dialog = function () {
         ...
         * @return {Promise}
         */
        this.showCodeBlockDialog = function ($editable, $dialog, langType) {
            return $.Deferred(function (deferred) {
                ...
                $codeBlockDialog.one('shown.bs.modal', function () {
                    $langType.val(langType).change(function () {
                        toggleBtn($codeBlockBtn, $langType.val());
                    }).trigger('keyup').trigger('focus');

                    $codeBlockBtn.click(function (event) {
                        event.preventDefault();

                        deferred.resolve($langType.val(), $showLineNumbers.is(':checked'));
    ...
    var EventHandler = function () {
        ...
        var commands = {
             ...
             * @param {Object} oLayoutInfo
             */
            showCodeBlockDialog: function (oLayoutInfo) {
                var $dialog = oLayoutInfo.dialog(),
                    $editable = oLayoutInfo.editable(),
                    langType = editor.getLanguageType($editable);

                editor.saveRange($editable);

                dialog.showCodeBlockDialog($editable, $dialog, langType).then(function (langType, isShowLineNumbers) {
                    editor.restoreRange($editable);
                    editor.insertCodeBlock($editable, langType, isShowLineNumbers);
                }).fail(function () {
                    editor.restoreRange($editable);
                });
            },
    ...
    var Renderer = function () {
        ...
        var tplButtonInfo = {
            ...
            codeBlock: function (lang) {
                return tplIconButton('fa fa-file-code-o icon-play', {
        ...
        var tplDialogs = function (lang, options) {
            ...
            var tplCodeBlockDialog = function () {
                var body = '<div class="form-group">' +
